...
local Cfg = {
    ...
    Aim = {
        Enabled=false, FOV=300, ShowFOV=true,
...
local function BestTarget()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        -- FOV gate only when FOV > 0
        if Cfg.Aim.FOV>0 and d>Cfg.Aim.FOV then continue end
...
-- ════════════════════════════════════════════════════════════
-- SILENT AIM — FIXED
-- detects weapon remotes by positional arg signature
-- ALWAYS fires normally when no target in FOV
-- ════════════════════════════════════════════════════════════
local oldNC
oldNC=hookmetamethod(game,"__namecall",function(self,...)
    ...
    and (method=="FireServer" or method=="InvokeServer") then
        ...
        -- detect positional args = sign of weapon remote
        local hasPos=false
        ...
        if hasPos then
            -- hit chance
            if Cfg.Aim.HitChance<100
            and math.random(1,100)>Cfg.Aim.HitChance then
                return oldNC(self,...)
            end
            -- RMB lock-on gate (PC only, off by default)
            if Cfg.Aim.LockOnKey
            and not UserInputService:IsMouseButtonPressed(
            ...
            if t then
                for i,v in ipairs(args) do
                    ...
                    elseif typeof(v)=="CFrame" then
                        args[i]=CFrame.new(t.pos)
                    elseif typeof(v)=="Instance" then
                        if v:IsA("BasePart") then args[i]=t.bone
                        elseif v:IsA("Player") then args[i]=t.player end
                    end
                end
                return oldNC(self,table.unpack(args))
            end
            -- no target — shoot normally, never block
            return oldNC(self,...)
        end
    end
    ...
    if gun then
        for _,v in ipairs(gun:GetDescendants()) do
            if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
                local n=v.Name:lower()
                if n:find("shoot") or n:find("fire") then
...
RunService.Heartbeat:Connect(function()
    ...
    savedGunPos=hrp.CFrame; lastGunTP=tick()
    ...
    task.spawn(function()
        ...
        if h and savedGunPos then h.CFrame=savedGunPos; savedGunPos=nil end
    end)
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if knife then
        ...
        if onScreen then
            KnifeArrow.PointA=Vector2.new(sp.X,sp.Y-14)
            KnifeArrow.PointB=Vector2.new(sp.X-8,sp.Y+6)
            KnifeArrow.PointC=Vector2.new(sp.X+8,sp.Y+6)
            KnifeArrow.Visible=true
            KnifeLabel.Position=Vector2.new(sp.X,sp.Y+10); KnifeLabel.Visible=true
        else
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        obj.Box.Visible=E and Cfg.ESP.Box; obj.Box.Size=Vector2.new(w,h)
        obj.Box.Position=Vector2.new(sx-w/2,sy-h/2); obj.Box.Color=col
        obj.Box.Thickness=Cfg.ESP.Thickness
        ...
        obj.Name.Visible=E and Cfg.ESP.NameTag; obj.Name.Text=p.Name
...
local function BuildGUI(platform)
    ...
    local Acc=Instance.new("Frame"); Acc.Size=UDim2.new(1,0,0,3)
    Acc.BackgroundColor3=Color3.fromRGB(200,30,30)
    Acc.BorderSizePixel=0; Acc.Parent=Win
    ...
    TL.TextXAlignment=Enum.TextXAlignment.Left; TL.Parent=Win

    local TB=Instance.new("Frame"); TB.Size=UDim2.new(1,0,0,tabH)
    TB.Position=UDim2.new(0,0,0,38)
    TB.BackgroundColor3=Color3.fromRGB(17,17,23)
    ...
    DL.BorderSizePixel=0; DL.Parent=Win

    local Ct=Instance.new("ScrollingFrame")
    Ct.Size=UDim2.new(1,0,1,-(42+tabH))
    Ct.Position=UDim2.new(0,0,0,42+tabH)
    Ct.BackgroundTransparency=1; Ct.BorderSizePixel=0
    Ct.ScrollBarThickness=3
    Ct.ScrollBarImageColor3=Color3.fromRGB(200,30,30)
    ...
    local function MakePage(name)
        ...
        pp.Parent=pg; Pages[name]=pg; return pg
    end
    ...
    for i,name in ipairs(Tabs) do
        ...
        btn.Text=name; btn.TextSize=mob and 13 or 11
    ...
    local function Sec(pg,text)
        ...
        f.Size=UDim2.new(1,-24,0,mob and 26 or 22)
    ...
    local function Tog(pg,label,tbl,key,cb)
        ...
        track.BorderSizePixel=0; track.Active=true; track.Parent=row
        ...
        knob.BorderSizePixel=0; knob.Active=true; knob.Parent=track
    ...
    local function Sld(pg,label,tbl,key,mn,mx,fmt,cb)
        ...
        lbl.Size=UDim2.new(0.65,0,0,mob and 24 or 20)
        lbl.BackgroundTransparency=1; lbl.Text=label
        lbl.TextColor3=Color3.fromRGB(195,195,210); lbl.TextSize=bTxt
        lbl.Font=Enum.Font.Gotham
        ...
        vL.Size=UDim2.new(0.35,0,0,mob and 24 or 20)
        ...
        vL.TextXAlignment=Enum.TextXAlignment.Right; vL.Parent=wrap
        ...
        track.BackgroundColor3=Color3.fromRGB(30,30,42)
        track.BorderSizePixel=0; track.Active=true; track.Parent=wrap
        Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
    ...
    local function DD(pg,label,options,tbl,key)
        ...
        wrap.Size=UDim2.new(1,-24,0,mob and 68 or 56)
        wrap.BackgroundTransparency=1; wrap.Active=true; wrap.Parent=pg
        local lbl=Instance.new("TextLabel")
        lbl.Size=UDim2.new(1,0,0,mob and 24 or 20)
        lbl.BackgroundTransparency=1; lbl.Text=label
        lbl.TextColor3=Color3.fromRGB(195,195,210); lbl.TextSize=bTxt
        lbl.Font=Enum.Font.Gotham
        lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Parent=wrap
        ...
        btn.TextSize=bTxt-1; btn.Font=Enum.Font.GothamBold
        btn.Active=true; btn.Parent=wrap
        ...
        dF.BackgroundColor3=Color3.fromRGB(22,22,30)
        dF.BorderSizePixel=0; dF.ZIndex=20; dF.Active=true; dF.Parent=wrap
        Instance.new("UICorner",dF).CornerRadius=UDim.new(0,5)
        for i,opt in ipairs(options) do
            local ob=Instance.new("TextButton")
            ob.Size=UDim2.new(1,0,0,oh)
            ob.Position=UDim2.new(0,0,0,(i-1)*oh)
            ob.BackgroundTransparency=1; ob.Text=opt
            ob.TextSize=bTxt-1; ob.Font=Enum.Font.Gotham; ob.ZIndex=21
    ...
    if not mob then
        Tog(P.AIM,"RMB Lock-On", Cfg.Aim,"LockOnKey")
    end
    ...
    Sec(P.AIM,"[ TUNING ]")
    Sld(P.AIM,"FOV Radius",      Cfg.Aim,"FOV",      30,500,"%d px")
    ...
    Tog(P.MOVE,"Fly  [WASD + Space/Ctrl]",Cfg.Movement,"Fly",function(s)
    ...
    Tog(P.EXTRA,"Anti-AFK",      Cfg.AntiAFK,"Enabled",function(s)
        ToggleAntiAFK(s)
    end)

    if mob then
        local MT=Instance.new("TextButton")
        MT.Size=UDim2.new(0,54,0,54)
        MT.Position=UDim2.new(1,-62,0.5,-27)
        MT.BackgroundColor3=Color3.fromRGB(200,30,30)
        MT.BorderSizePixel=0; MT.Text="☰"
        MT.TextColor3=Color3.fromRGB(255,255,255)
        MT.TextSize=28; MT.Font=Enum.Font.GothamBold
        MT.ZIndex=50; MT.Parent=SG
    ...
    TweenService:Create(Win,TweenInfo.new(0.4),
...
task.spawn(function()
    ...
    TweenService:Create(PickerFrame,TweenInfo.new(0.4),
        {BackgroundTransparency=0}):Play()
